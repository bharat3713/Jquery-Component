(function ($) {
    $.fn.abcGrid = function (options) {
        ...
        for (var i = 0; i < totalRecord; i++) {
            myD.push([settings.dataSource[i].Threat, settings.dataSource[i].Name, settings.dataSource[i].Id, settings.dataSource[i].DOB, settings.dataSource[i].ArrestDate, settings.dataSource[i].Location]);
        }
        var theTable = "";
        for (var j = 0; j < totalRecord; j++) {
            ...
            for (var k = 0; k < numberOfKeys; k++) {
                if (k == 0) {
                    ...
                    theTable += "<div class='tcol " + settings.columeName[k] + "' style='width:calc(" + settings.columeWidth[k] + " - 14px)'><a href='#' onclick='showInmateData();'>" + myD[j][k] + "</div>";
                }
            }
            theTable += '</div>';
        }
